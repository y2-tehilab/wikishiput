...
export default function Header({onSearch, entries}) {
  ...
  const initEntries = async () => {
    const allEntriesResponse = entries?.length ? entries : await getAllEntries();
    const options = allEntriesResponse.map(({item1, item2}) => ({text: item2, value: item1}))
    setOptions(options);
  };
  ...
  return (
    <div className={styles.header}>
      <Logo />
      {options?.length ? <SearchInput options={options} searchClicked={onSearch ? onSearch : null}/> : null}
